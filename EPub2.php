...
class EPub {
    ...
    function setCoverImage($fileName, $imageData = NULL, $mimetype = NULL, $title = 'CoverPage', $addToIndex = FALSE) {
        ...
        if ($addToIndex) {
        	$this->addChapter($title, "CoverPage.xhtml", $coverPage);
        } else {
        	$this->addReferencePage($title, "CoverPage.xhtml", $coverPage, "cover");
        }
	...
	private function finalizeTOC() {
        ...
        . "</head>\n"
        . "<body>\n"
        . "<div class=\"" . $this->tocCSSClass . "-title\">" . $this->tocTitle . "</div>\n<div";
        ...
        foreach ($this->referencesOrder as $item => $descriptive) {
            if ($item === "text") {
                foreach ($this->ncx->chapterList as $chapterName => $navPoint) {
                    ...
                    $level = $navPoint->getLevel() - 1;
                    $level = ($level >= 1) ? $level : 1;
                    $level = ($level <= 6) ? $level : 6;
                    $tocData .= "\t<div class=\"" . $this->tocCSSClass . "-level-" . $level . "\"><a href=\"" . $fileName . "\">" . $chapterName . "</a></div>\n";
                }
            } else if ($this->tocAddReferences === TRUE) {
                if (array_key_exists($item, $this->ncx->referencesList)) {
                    $tocData .= "\t<div class=\"" . $this->tocCSSClass . "-level-1\"><a href=\"" . $this->ncx->referencesList[$item] . "\">" . $descriptive . "</a></div>\n";
                } else if ($item === "toc") {
                    $tocData .= "\t<div class=\"" . $this->tocCSSClass . "-level-1\"><a href=\"TOC.xhtml\">" . $this->tocTitle . "</a></div>\n";
                } else if ($item === "cover" && $this->isCoverImageSet) {
                    $tocData .= "\t<div class=\"" . $this->tocCSSClass . "-level-1\"><a href=\"CoverPage.xhtml\">" . $descriptive . "</a></div>\n";
                }
            }
        }
        $tocData .= "</div>\n</body>\n</html>\n";
